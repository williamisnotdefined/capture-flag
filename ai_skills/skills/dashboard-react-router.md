...
Use this skill when adding or changing routing in `apps/dashboard`.

## Rules

- Use React Router for page navigation and route rendering.
- Keep the router setup near the dashboard entrypoint, with route UI in page or layout components.
- Keep `QueryClientProvider` outside the router so all routes share the same React Query client.
- Prefer `Link` and `NavLink` over plain anchors for internal dashboard navigation.
...
- Run `npm --workspace @capture-flag/dashboard run build` after route changes.
