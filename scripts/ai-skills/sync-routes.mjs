...
const registrySchemaPath = "ai_skills/registry.schema.json";
...
const approvedReferenceRoots = [
  "ai_skills/rules",
  "ai_skills/architecture",
  "ai_skills/glossary",
  "ai_skills/examples",
];
...
function cursorRoute(skill, routePath, context) {
  ...
  if (alwaysApply && skill.global !== true) {
    throw new Error(
      `${skill.name}: toolConfig.cursor.alwaysApply can be true only for global skills.`,
    );
  }

  if (!alwaysApply && cursorConfig.alwaysApply !== false) {
    throw new Error(
      `${skill.name}: toolConfig.cursor.alwaysApply must be false unless skill.global is true.`,
    );
  }
...
function validateRegistrySchemaShape(registry) {
  validateAllowedKeys(registry, "registry", [
    "$schema",
    "version",
    "canonicalRoot",
    "tools",
  ...
  for (const [skillIndex, skill] of registry.skills.entries()) {
    ...
    validateAllowedKeys(skill.toolConfig?.cursor, `${skillField}.toolConfig.cursor`, [
      "globs",
      "alwaysApply",
    ]);
    requiredString(skill.toolConfig?.cursor?.globs, `${skillField}.toolConfig.cursor.globs`);
    validateBoolean(
      skill.toolConfig?.cursor?.alwaysApply,
      `${skillField}.toolConfig.cursor.alwaysApply`,
    );
...
function parseReadFirstReferences(skill, content) {
  ...
  for (const line of body.split("\n")) {
    ...
    if (!referenceMatch) {
      throw new Error(
        `${skill.name}: ## Read First must contain only backticked reference bullets.`,
      );
    }
...
function validateReadFirstReferences(skill, content, registryReferences) {
  ...
  for (const [index, referencePath] of registryReferences.entries()) {
    if (readFirstReferences[index] !== referencePath) {
      throw new Error(
        `${skill.name}: ## Read First must match registry references in the same order.`,
      );
    }
  }
}
...
async function validateRegisteredSkillFiles(registry, canonicalRoot) {
  const registeredSkillFiles = new Set(
    registry.skills.map((skill) =>
      requiredRelativePath(skill.canonicalPath, `${skill.name}.canonicalPath`),
    ),
  );

  for (const skillPath of await listFilesRecursive(canonicalRoot)) {
    if (skillPath.endsWith(".md") && !registeredSkillFiles.has(skillPath)) {
      throw new Error(`${skillPath} is not registered in ai_skills/registry.json.`);
    }
  }
}

async function validateExamples() {
  for (const examplePath of await listFilesRecursive("ai_skills/examples")) {
...
async function main() {
  ...
  if (staleRoutes.length > 0 || orphanRoutes.length > 0) {
    ...
    const orphanMessage =
      orphanRoutes.length > 0 ? `Orphan generated files: ${orphanRoutes.join(", ")}` : "";
    throw new Error(
      `AI skill route files are out of sync. Run npm run ai-skills:sync. ${[
        staleMessage,
        orphanMessage,
      ]
        .filter(Boolean)
