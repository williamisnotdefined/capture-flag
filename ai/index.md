...
The goal is an AI knowledge base for Capture Flag, not a folder of generic prompts. Reusable project knowledge lives in `rules`, `architecture`, `glossary`, and `examples`. Task workflows live in `skills` and reference that reusable context through `ai/registry.json`.
...
`ai/registry.schema.json` documents the registry shape for editors and mirrors the structure enforced by `npm run ai:check`.
...
`npm run ai:sync` compiles every canonical skill plus its references into tool-specific route files. Generated route files duplicate compiled context intentionally so tools receive the same project knowledge without manually copying content.
...
- `npm run ai:check` verifies canonical skill files, referenced context files, generated route content, and orphan generated route files.
